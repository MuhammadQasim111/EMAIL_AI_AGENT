...
# Imports from your email-agent project's packages
from agents.agent import Agent
from agents.runner import Runner
from models.email_models import Email, EmailContext
from magents.manager_agent import ManagerAgent # Import the actual ManagerAgent class
# from tools.email_tools import enable_verbose_logging # Uncomment if you have this function and want to use it
# You would also import save_emails_to_human_review, save_emails_to_automation, get_statistics
# if you were directly testing the tools here. The ManagerAgent itself has them.
...
# If you have an enable_verbose_logging function in email_tools.py, uncomment the next line
...
async def test_manager_agent():
    ...
    test_logger.info("Creating test manager agent instance")
    # Use the actual ManagerAgent class
    manager_agent_instance = ManagerAgent()
    
    # Prepare email data for the model
    # The ManagerAgent.process_emails method expects a list of dictionaries
    email_data = []
    ...
    # Run the manager agent's processing method
    test_logger.info("Running manager agent test...")
    # The ManagerAgent.process_emails method encapsulates the Runner.run call
    agent_output = await manager_agent_instance.process_emails(email_data, context=context)
    ...
    print("\n=== Agent Response ===")
    print(agent_output) # This is the final output from the agent's processing
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            print("\n=== All Emails ===")
            for i, email in enumerate(context.emails.values()): # Iterate over values for all emails
                print(f"\nEmail {i+1}:")
                print(f"ID: {email.id}")
                print(f"Subject: {email.subject}")
                print(f"From: {email.sender}")
            ...
            if context.human_review_results:
                ...
                print("No human review results recorded.")
            
            # Show the automation results
            print("\n=== Automation Results ===")
            if context.automation_results:
                ...
                print("No automation results recorded.")
